/*	CellBlocks_v12.ijm
	***********************
	...
	Date Last Modified:	December 3rd, 2021 
...
var cells_threshold					= "Triangle";								//	threshold method for segmentation of cells 
var dir								= "";										//	directory
...
var cells_channel					= 4;										//	channel used for segmentation of cells 
...
var nuclei_filter_scale				= 0;										// 	nuclei_filter_scale radius for nuclei
...
var pixel_size						= 0.571;									//	pixel size (µm)
...
{
	setOptions();
	Dialog.createNonBlocking("CellBlocks_v09 Settings");
	...
	Dialog.addToSameRow();
	Dialog.addChoice("To (primary):", objects, primary_objects);
	Dialog.setInsets(0,0,0);
	...
	Dialog.setInsets(0,0,0);
	Dialog.addMessage("Cells are segmented by thresholding a selected channel of a cell stain or if absent (set channel to 0), by dilating nuclear ROI seeds.\nRegion growing is performed for a defined number of iterations, or if iterations is set to 0, sheer Voronoi tesselation is applied. \n", 12, "#999999");
	Dialog.setInsets(0,0,0);
...
function segmentNuclei(id,c,sel)
{
	...
	{
		selectImage(cid);
		run("Analyze Particles...", "size=1-Infinity circularity=0-1.00 show=Masks clear include");
...
function segmentRegions(id, mid, c, iterations)
{
	...
	if(c > 0)
	{
		...
		cid = getImageID;
		selectImage(cid);
		run("Gaussian Blur...", "sigma="+round(0.8/pixel_size));
		...
		else setAutoThreshold(cells_threshold+" dark"); 
		getThreshold(mit,mat); print("Cell Threshold:",mit,mat);
		setOption("BlackBackground", false);
		run("Convert to Mask");
		run("Fill Holes");
		imageCalculator("AND create", "voronoi","copy");
		selectImage(cid); close; 
		selectImage(vid); close;
		selectWindow("Result of voronoi");
		vid = getImageID;
	...
	run("Select None");
	if(!exclude_nuclei){roiManager("reset"); rmc=0;}
	selectImage(vid);
	...
	for(i=rmcb-1;i>=rmc;i--)
	{
		...
		if(max==0){roiManager("delete");} 				// no nuc retained
