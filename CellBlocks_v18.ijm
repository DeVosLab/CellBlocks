/*	CellBlocks_v18.ijm
	*************4**********
	...
	Date Last Modified:	March 9th, 2023 
...
macro "Analyse Single Image Action Tool - C888 T5f161"
{
	...
	if(nuclei_nr>0 && segment_cells)
	{
		...
		if(cell_nr>0)
		{
			roiManager("Save",cells_roi_set);
			if(nuclei_nr!=cell_nr)checkRoiMatch(nuclei_nr,cell_nr);
		}
...
{
	setOptions();
	Dialog.createNonBlocking("CellBlocks_v17 Settings");
	...
	cells_diameter					= Dialog.getNumber();
...
function segmentRegions(id, mid, c, iterations){
	...
	roiManager("Fill");
	run("Convert to Mask");
	imageCalculator("OR create", "Cell_ROI","posnuclei"); // fuse cell and nuclei image to avoid non-overlapping ROI
	selectImage(vid); close;
	...
	//check for duplicates (pieces of the same cell roi) and fuse
	rmcb = roiManager("count");  // number of cell regions after filtering out regions that have no nucleus
	print(rmcb-rmc,"Number of detected cell regions that overlap with a nucleus"); 
	for(i=rmcb-1;i>=rmc;i--)
	{
		roiManager("select",i); 
		roi_name_a = Roi.getName();
		rmcb_temp = roiManager("count"); //Activate roi count
		for(j=rmcb_temp-1;j>=rmc;j--)
		{			
			roiManager("select", j); 
			roi_name_b = Roi.getName(); 
			if (matches(roi_name_a, roi_name_b)&&(i!=j))
			{
				couple = newArray(i,j);
				roiManager("select",couple); 
				roiManager("OR"); 
				roiManager("Add"); //add the combined roi
				roiManager("select",roiManager("count")-1);
				roiManager("Rename",roi_name_a);
				roiManager("select",couple); 
				roiManager("Delete"); // remove the couple
			}
		}
	} 
	...
	if(rmcc>rmc)
	{
		...
		else if(!exclude_nuclei)
		{
			...
			roiManager("Delete"); //remove all nuclei ROIsand retain cell ROIs
		} 
		run("Select None");
...
function checkRoiMatch(nuclei_nr,cell_nr)
{
	roiManager("reset");
	nuclei_ROI = newArray(nuclei_nr);
	cell_ROI = newArray(cell_nr);
	delete_ROI = newArray(0); //array of ROIs that need deletion
	roiManager("Open",nuclei_roi_set);
	for(i=0;i<nuclei_nr;i++)
	{
		roiManager("select",i); 
		nuclei_ROI[i] = Roi.getName();
	}
	roiManager("reset");
	...
	{
		roiManager("select",i); 
		cell_ROI[i] = Roi.getName();
	}
	roiManager("reset");
	if(nuclei_nr>cell_nr)
	{
		print("Nuc Nr > Cell Nr");
		for(i=0;i<nuclei_nr;i++)
		{
			check = Array.filter(cell_ROI,nuclei_ROI[i]);
			if(lengthOf(check)==0)  // no corresponding roi
			{
				delete_ROI = Array.concat(delete_ROI,i);
				print("Deleted", nuclei_ROI[i],"as there is no corresponding cell ROI");
			}
		}
		roiManager("Open",nuclei_roi_set);
		roiManager("select",delete_ROI); 
		roiManager("Delete");
	...
	else if(cell_nr>nuclei_nr)
	{
		print("Cell Nr > Nuc Nr");
		for(i=0;i<cell_nr;i++)
		{
			check = Array.filter(nuclei_ROI,cell_ROI[i]);
			if(lengthOf(check)==0)  // no corresponding roi
			{
				delete_ROI = Array.concat(delete_ROI,i);
				print("Deleted", cell_ROI[i],"as there is no corresponding nuclei ROI")
			}
		}
		roiManager("Open",cell_roi_set);
		roiManager("select",delete_ROI); 
		roiManager("Delete");
		rmc=roiManager("count");
		for(i=0;i<rmc;i++)
	...
	print("Cell and Nuc ROI matched");
}
...
function analyzeRegions(id)
{
	...
	if(File.exists(spots_a_roi_set))
	{
		...
		for(j=0;j<spot_nr;j++)
		{
			min = getResult("Min",j);
			max = getResult("Max",j);
			if(max>0){nindices[j] = max; cindices[j] = max;}
...
function summarizeResults()
{
	...
	{	
		for(r=0;r<nnr;r++){indices[r]=getResult("Cell",r)-1;} // once in a while a cell index is different from a nuclear index
		//Array.print(indices);
		IJ.renameResults("Results","Temp");
		...
		for(r=0;r<nnr;r++)
		{
			for(s=0;s<resultLabels.length;s++)
			{
				...
				setResult("Cell_SC"+cells_channel+"_"+resultLabels[s],r,p); // Label all cytoplasmic measured parameters with a "Cell" prefix
			}
		}
		updateResults;
		selectImage(matrix); close;
		...
		for(s=0;s<resultLabels.length;s++)
		{
			if(resultLabels[s] != "Nucleus" && resultLabels[s] != "Cell")
			{
				...
				for(r=0;r<nnr;r++)
				{
					...
					if(segment_cells)
					{
						setResult("Spot_SC"+spots_a_channel+"_NrPerCell",r,cnumber[indices[r]]);
						setResult("Spot_SC"+spots_a_channel+"_"+resultLabels[s]+"_SumPerCell",r,cvalues[indices[r]]);
						setResult("Spot_SC"+spots_a_channel+"_"+resultLabels[s]+"_MeanPerCell",r,cvalues[indices[r]]/cnumber[indices[r]]);
					}
				}
			}
		}
		...
		for(s=0;s<resultLabels.length;s++)
		{
			if(resultLabels[s] != "Nucleus" && resultLabels[s] != "Cell")
			{
				...
				for(r=0;r<snr;r++)
				{
					...
					else if(!exclude_nuclei && cindices[r]>=0)
					{
						cvalues[cindices[r]] += p;	
						cnumber[cindices[r]] += 1;	
					}	
				...
				for(r=0;r<nnr;r++)
				{
					...
					if(segment_cells)
					{
						...
						setResult("Spot_SC"+spots_b_channel+"_"+resultLabels[s]+"_SumPerCell",r,cvalues[indices[r]]);
						setResult("Spot_SC"+spots_b_channel+"_"+resultLabels[s]+"_MeanPerCell",r,cvalues[indices[r]]/cnumber[indices[r]]);
					}
				}
			}
		}
			...
			if(resultLabels[s] != "Nucleus" && resultLabels[s] != "Cell")
			{
				...
				for(r=0;r<nnr;r++)
				{
					...
					if(segment_cells)
					{
						...
						setResult("Fold_SC"+folds_channel+"_"+resultLabels[s]+"_SumPerCell",r,cvalues[indices[r]]);
						setResult("Fold_SC"+folds_channel+"_"+resultLabels[s]+"_MeanPerCell",r,cvalues[indices[r]]/cnumber[indices[r]]);
					}
				}
			}
		}
